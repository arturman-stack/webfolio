...
const TextLoop = ({
  ...
  const effectiveFontSize = isCompact ? fontSize * 3 : fontSize;

  const textStyle = useMemo(
    () => ({
      fontSize: `${effectiveFontSize}vw`,
      fontWeight,
      letterSpacing: `${letterSpacing}vw`,
      fontFamily: 'sans-serif'
    }),
    [effectiveFontSize, fontWeight, letterSpacing]
  );
  ...
  }, [d, unit, effectiveFontSize, fontWeight, letterSpacing]);
  ...
  return (
    <div ref={rootRef} className={`relative w-full h-full overflow-hidden ${className}`.trim()} style={style}>
      ...
      >
        ...
        <text className="select-none" style={textStyle} fill={color} dominantBaseline="central" aria-hidden="true">
          <textPath style={{fontFamiliy: "sa"}} ref={tailRef} href={`#${pathId}`} startOffset={0} textLength={fitLength} lengthAdjust="spacing">
            {loopText}
          </textPath>
        </text>
      </svg>
